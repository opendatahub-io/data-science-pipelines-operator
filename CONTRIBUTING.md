...
#### Optional: Deploy DSPO with non-master changes
By default, the commands above deploy an image built from the DSPO `main` branch. To use deploy DSPO with local changes, run the following command to build and push the custom image to your image repository.
...
#### Optional: Deploy DSPA with non-master changes
- Build and push DSP images to `IMG_REGISTRY`
