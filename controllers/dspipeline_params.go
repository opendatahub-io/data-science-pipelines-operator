...
func (p *DSPAParams) ExtractParams(ctx context.Context, dsp *dspa.DataSciencePipelinesApplication, client client.Client, log logr.Logger) error {
	...
	if p.APIServer != nil {
		...
		kfpLauncherImageFromConfig := config.GetStringConfigWithDefault(APIServerKFPLauncherImagePath, config.DefaultImageValue)
		driverImageFromConfig := config.GetStringConfigWithDefault(APIServerDriverImagePath, config.DefaultImageValue)
		...
		setStringDefault(kfpLauncherImageFromConfig, &p.APIServer.KFPLauncherImage)
		setStringDefault(driverImageFromConfig, &p.APIServer.DriverImage)
