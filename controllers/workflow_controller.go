...
func (r *DSPAReconciler) ReconcileWorkflowController(dsp *dspav1.DataSciencePipelinesApplication,
	...
	if err != nil {
		log.Info("Unable to parse Argo Workflows Controller management state, using default value: %s", config.DefaultArgoWorkflowsControllersManagementState)
	...
	default:
		log.Info(fmt.Sprintf("Invalid management state for WorkflowController Resources: %s", argoWorkflowsControllerManagementState))
	}
