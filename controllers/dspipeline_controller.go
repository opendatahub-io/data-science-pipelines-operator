...
func (r *DSPAReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		databaseReady.Status = metav1.ConditionFalse
		databaseReady.Reason = config.DataBaseFailedToDeploy
		databaseReady.LastTransitionTime = metav1.Now()
		return ctrl.Result{}, err
	}

	if r.isDeploymentAvailable(ctx, dspa, "mariadb") && databaseReady.Status == metav1.ConditionFalse {
	...
	if err != nil {
		objectStorageReady.Status = metav1.ConditionFalse
		objectStorageReady.Reason = config.ObjectStorageFailedToDeploy
		objectStorageReady.LastTransitionTime = metav1.Now()
		return ctrl.Result{}, err
	}

	if r.isDeploymentAvailable(ctx, dspa, "minio") && objectStorageReady.Status == metav1.ConditionFalse {
	...
	if err != nil {
		apiServerReady.Status = metav1.ConditionFalse
		apiServerReady.LastTransitionTime = metav1.Now()
		return ctrl.Result{}, err
	}

	if r.isDeploymentAvailable(ctx, dspa, "ds-pipeline") && apiServerReady.Status == metav1.ConditionFalse {
	...
	if err != nil {
		persistenceAgentReady.Status = metav1.ConditionFalse
		persistenceAgentReady.LastTransitionTime = metav1.Now()
		return ctrl.Result{}, err
	}

	if r.isDeploymentAvailable(ctx, dspa, "ds-pipeline-persistenceagent") && persistenceAgentReady.Status == metav1.ConditionFalse {
	...
	if err != nil {
		scheduledWorkflowReady.Status = metav1.ConditionFalse
		scheduledWorkflowReady.LastTransitionTime = metav1.Now()
		return ctrl.Result{}, err
	}

	if r.isDeploymentAvailable(ctx, dspa, "ds-pipeline-scheduledworkflow") && scheduledWorkflowReady.Status == metav1.ConditionFalse {
	...
	if err != nil {
		userInterfaceReady.Status = metav1.ConditionFalse
		userInterfaceReady.LastTransitionTime = metav1.Now()
		return ctrl.Result{}, err
	}

	if r.isDeploymentAvailable(ctx, dspa, "ds-pipeline-ui") && userInterfaceReady.Status == metav1.ConditionFalse {
	...
	if ((apiServerReady.Status == metav1.ConditionTrue) &&
		...
		if !reflect.DeepEqual(conditions, dspa.Status.Conditions) {
