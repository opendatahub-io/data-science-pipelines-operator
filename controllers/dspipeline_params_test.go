...
func TestBuildCompiledPipelineSpecPatch(t *testing.T) {
	tt := []struct {
		name           string
		params         DSPAParams
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			result := tc.params.buildCompiledPipelineSpecPatch()

			if tc.expectedPatch != "" {
				assert.Equal(t, tc.expectedPatch, result)
			} else if tc.expectedFields != nil {
				// Verify the JSON contains expected fields
				var actualFields map[string]interface{}
				err := json.Unmarshal([]byte(result), &actualFields)
				require.NoError(t, err)
