...
### ✅ Required Action:
Comment `/integration-tests-ok` on this PR **only after** completing the verification steps below.

### 📝 Steps:

1. **Check for corresponding changes in data-science-pipelines repository:**
   - Verify if there are any related changes in [opendatahub-io/data-science-pipelines](https://github.com/opendatahub-io/data-science-pipelines)
   - If there are corresponding changes, ensure they have been tested and merged to the **stable** branch

2. **Ensure integration tests were run on data-science-pipelines stable branch:**
   - Verify that the [stable-merge-check.yml workflow](https://github.com/opendatahub-io/data-science-pipelines/blob/master/.github/workflows/stable-merge-check.yml) was executed successfully for any related changes
   - Confirm that the integration tests passed against the latest ODH nightly build
   - Check that any related PRs in data-science-pipelines have the `integration-tests-verified` label

3. **Run integration tests for data-science-pipelines-operator:** (If 1 and 2 are not true)
   - Set up OpenShift cluster with latest ODH nightly build
   - Deploy the Data Science Pipelines Operator from this PR's branch
   - Test core functionality including:
     * DSPA deployment and configuration
     * Pipeline execution and scheduling
     * Integration with ODH components
     * Compatibility with existing pipeline samples

4. **Run integration tests for data-science-pipelines-operator:** (If 1 and 2 are not true)
    ...
    8. Uncheck RUN_TESTS
    9. Run the job
    10. Once the deployment is DONE and your cluster is available, update the DSPO version by following the instructions [here](https://github.com/opendatahub-io/opendatahub-operator/blob/main/hack/component-dev/README.md)
    11. On your local, run the Smoke tests via the following docker (or podman equivalent command):
        ```
        ...
        ```
    12. Now go back to Jenkins to run tests:
    ...
    13. Repeat the above steps but this time check `ENABLE_FIPS_IN_CLUSTER` checkbox to create a FIPS cluster
5. Comment `/integration-tests-ok` on this PR to add the verification label
6. This workflow check will automatically pass once the label is added

### 🔗 Related Repositories:
- **Data Science Pipelines**: [opendatahub-io/data-science-pipelines](https://github.com/opendatahub-io/data-science-pipelines)
- **Workflow Reference**: [stable-merge-check.yml](https://github.com/opendatahub-io/data-science-pipelines/blob/master/.github/workflows/stable-merge-check.yml)

### ✅ Final Step:
Comment `/integration-tests-ok` on this PR to add the verification label and allow merge to proceed.

### 🔒 Authorization:
Only organization members and owners can use the `/integration-tests-ok` command.

---
*This requirement ensures production stability by verifying integration tests and cross-repository compatibility.*"""
...
1. Verify any corresponding changes in [data-science-pipelines](https://github.com/opendatahub-io/data-science-pipelines) have been tested
2. If not, then run integration tests for data-science-pipelines-operator with the new commits
3. Ensure cross-repository compatibility with latest versions
4. Comment `/integration-tests-ok` on this PR after confirming all tests pass
...
Once you comment `/integration-tests-ok`, the label will be re-added and this workflow will automatically pass."""
    ...
    if has_label:
        ...
        if label_was_removed:
            ...
            print("   2. Comment '/integration-tests-ok' on this PR after tests pass")
            print("   3. This workflow will automatically re-run and pass")
        else:
