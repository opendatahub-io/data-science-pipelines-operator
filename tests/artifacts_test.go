...
func (suite *IntegrationTestSuite) TestFetchArtifacts() {
	suite.T().Run("Should successfully fetch and download artifacts", func(t *testing.T) {
		...
		log.Printf("Using MinIO namespace: %s", suite.MinioNamespace)
		...
		log.Printf("Port forwarding to assigned MinIO namespace: %s", minioNamespace)
		log.Printf("Using MinIO service name: %s", minioServiceName)

		// Run `kubectl get svc` to confirm MinIO service exists
		cmdCheck := exec.Command("kubectl", "get", "svc", "-n", MinioNamespace)
		output, err := cmdCheck.CombinedOutput()
		log.Printf("MinIO service check output:\n%s", string(output))
		if err != nil {
			log.Printf("Error checking MinIO service: %v", err)
		}
		...
		go func() {
			err := cmd2.Wait()
			if err != nil {
				log.Printf("The Minio port forward failed: %v\n", err)
			}
		}()

		defer func() {
			log.Println("Waiting for all tests to complete before terminating port-forwarding...")
			...
			log.Println("Port-forwarding process terminated.")
		}()
		...
		for _, artifact := range responseArtifactsData.Artifacts {
			...
			httpClient := &http.Client{
				Transport: &http.Transport{
					TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, //Fix TLS verification error
				},
			}
