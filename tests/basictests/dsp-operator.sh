...
function create_and_verify_data_science_pipelines_resources() {
    ...
    echo "Fetching pods: "
    oc get pods -n ${DSPAPROJECT} -l component=data-science-pipelines
    sleep 180m
...
function check_custom_resource_conditions() {
    ...
    os::cmd::try_until_text "oc get -n ${DSPAPROJECT} datasciencepipelinesapplication example -o jsonpath='{.status.conditions[6].status}'" "True" $odhdefaulttimeout $odhdefaultinterval

    ## Given general condition is good, is expected that other component conditions are good
    # DataBaseReady
    os::cmd::try_until_text "oc get -n ${DSPAPROJECT} datasciencepipelinesapplication example -o jsonpath='{.status.conditions[0].status}'" "True" $odhdefaulttimeout $odhdefaultinterval
    # ObjectStorageReady
    os::cmd::try_until_text "oc get -n ${DSPAPROJECT} datasciencepipelinesapplication example -o jsonpath='{.status.conditions[1].status}'" "True" $odhdefaulttimeout $odhdefaultinterval
    # ApiServerReady
    os::cmd::try_until_text "oc get -n ${DSPAPROJECT} datasciencepipelinesapplication example -o jsonpath='{.status.conditions[2].status}'" "True" $odhdefaulttimeout $odhdefaultinterval
    # PersistenceAgentReady
    os::cmd::try_until_text "oc get -n ${DSPAPROJECT} datasciencepipelinesapplication example -o jsonpath='{.status.conditions[3].status}'" "True" $odhdefaulttimeout $odhdefaultinterval
    # ScheduledWorkflowReady
    os::cmd::try_until_text "oc get -n ${DSPAPROJECT} datasciencepipelinesapplication example -o jsonpath='{.status.conditions[4].status}'" "True" $odhdefaulttimeout $odhdefaultinterval
    # UserInterfaceReady
    os::cmd::try_until_text "oc get -n ${DSPAPROJECT} datasciencepipelinesapplication example -o jsonpath='{.status.conditions[5].status}'" "True" $odhdefaulttimeout $odhdefaultinterval
}
